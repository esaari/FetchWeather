 ...
 */
const mockGetFromEndpoint = async (baseUri, queryParam, key) => {
...
test.describe('getWeather Function Integration Tests', () => {
    ...
    test('should process valid zip codes correctly', async () => {
        ...
        locations.forEach((loc, index) => {
            expect(logOutput[index * 2]).toContain(`The current weather condition in ${fullValidResponse.name} is supposedly ${fullValidResponse.weather[0].main} with ${fullValidResponse.weather[0].description}`);
            expect(logOutput[index * 2 + 1]).toContain(`${fullValidResponse.name} has a Latitude of: ${fullValidResponse.coord.lat} and a Longitude of: ${fullValidResponse.coord.lon}`);
        });
    ...
    test('should handle invalid locations gracefully', async () => {
        ...
        const mockGetFromEndpointInvalid = async (baseUri, queryParam, key) => {
    ...
    test('should handle mixed valid and invalid locations', async () => {
        ...
        const mockGetFromEndpointMixed = async (baseUri, queryParam, key) => {
