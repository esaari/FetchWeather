...
test.describe('validateInput(string[]) Unit Tests', () => {
    ...
    test('should throw an error when locations is undefined', () => {
        expect(() => validateInput(undefined)).toThrowError(
            'Expected a list of locations, but received: undefined'
        );
    });
    ...
    test('should throw an error when any location is invalid', () => {
        ...
        invalidLocations.forEach((loc) => {
            expect(() => validateInput([loc])).toThrowError(
                `ERROR: ${loc} at index 0 must be a valid 5-digit zip code or City,State`
            );
        });
    });
...
test.describe('validateLength(string[]) Unit Tests', () => {
    ...
    test('should throw an error when locations is undefined', () => {
        expect(() => validateLength(undefined)).toThrowError(
            'Expected a list of locations, but received: undefined'
        );
    });

    test('should throw an error when no locations are provided', () => {
        expect(() => validateLength([])).toThrowError(
            'No locations provided. Please specify at least one location.'
        );
    });

    test('should throw an error when more than 10 locations are provided', () => {
        const tooManyLocations = Array(11).fill('12345');
        expect(() => validateLength(tooManyLocations)).toThrowError(
            'ERROR: Too many arguments. Please input no more than 10 locations.'
        );
    });
